---
description: >-
  Creates and manages Salesforce approval processes including multi-step approvals, email alerts, field updates, and outbound messages. Use when building approval workflows, configuring approval steps and actions, or troubleshooting approval routing. Do NOT use for Flow-based approvals (use sf-flow) or Apex-only automation (use sf-apex).
---
...
```
Record Submitted
  â†’ Entry Criteria (filter)
  â†’ Step 1: Approval (approver assignment)
    â†’ Approved â†’ Step 2 (optional)
    â†’ Rejected â†’ Rejection Actions
  â†’ Final Approval Actions
    â†’ Field Update (Status = 'Approved')
    â†’ Email Alert to submitter
  â†’ Final Rejection Actions
    â†’ Field Update (Status = 'Rejected')
    â†’ Email Alert to submitter
```
