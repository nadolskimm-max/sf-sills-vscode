---
description: >-
  Creates test specifications and validation strategies for Agentforce agents. Use when writing agent test scenarios, building conversation test scripts, validating agent behavior against persona rules, or running agentic fix loops. Do NOT use for Apex unit tests (use sf-testing) or agent configuration (use sf-ai-agentforce).
---
...
5. Run agentic fix loops (test â†’ diagnose â†’ fix â†’ retest)
...
3. DIAGNOSE root cause:
   - Action returned wrong data? â†’ Fix Apex/Flow
   - Wrong topic selected? â†’ Adjust topic description/keywords
   - Guardrail not enforced? â†’ Update system instructions
   - Tone mismatch? â†’ Refine persona instructions
4. APPLY fix
