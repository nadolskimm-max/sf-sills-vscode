---
description: >-
  Creates and reviews Salesforce Flows (Record-Triggered, Screen, Scheduled, Autolaunched, Platform Event-Triggered). Use when building or reviewing Flow XML, designing flow logic, handling bulk operations in flows, or troubleshooting flow errors. Do NOT use for Apex triggers (use sf-apex) or Process Builder (deprecated â€” migrate to Flow).
applyWhen: "**/*.flow-meta.xml"
---
...
- Runs before commit â€” no DML count
...
```
[Entry Criteria] â†’ [Get Records (before loop)] â†’ [Loop]
    â†’ [Assignment (add to collection)] â†’ [End Loop]
    â†’ [Update Records (collection)] â†’ [Fault Path]
```
