---
description: >-
  Guides architectural decisions on which Salesforce automation tool to use: Flow vs Apex vs Scheduled vs Platform Events. Use when deciding between automation approaches, migrating from Process Builder/Workflow Rules, planning automation architecture, or resolving order-of-execution conflicts. Do NOT use for building specific automations (use sf-flow, sf-apex, sf-async-patterns).
---
...
â”śâ”€â”€ YES â†’ Before-Save Flow (fastest, no DML cost)
â””â”€â”€ NO
    Does it need to create/update related records?
    â”śâ”€â”€ YES â†’ After-Save Flow (declarative) or Apex Trigger (complex)
    â””â”€â”€ NO
        Does it need callouts?
        â”śâ”€â”€ YES â†’ Queueable Apex or Platform Event â†’ subscriber
        â””â”€â”€ NO
            Does it need to run on a schedule?
            â”śâ”€â”€ YES â†’ Scheduled Flow or Schedulable Apex
            â””â”€â”€ NO
                Is the logic very complex (50+ decision paths)?
                â”śâ”€â”€ YES â†’ Apex Trigger + Service class
                â””â”€â”€ NO â†’ After-Save Flow
```

## Deprecated Tools â€” Migration Guide
...
### Process Builder â†’ Flow Migration

```
For each Process Builder:
1. Document all criteria nodes and actions
2. Create Record-Triggered Flow on same object
3. Replicate entry criteria â†’ Flow entry conditions
4. Replicate Immediate Actions:
   - Field Update â†’ Assignment element (before-save) or Update Records (after-save)
   - Email Alert â†’ Send Email action
   - Create Record â†’ Create Records element
   - Apex â†’ Action element (Invocable Apex)
5. Replicate Scheduled Actions â†’ Scheduled Path in Flow
...
Recommended pattern:
- **One trigger** per object â†’ delegates to handler classes (TAF)
