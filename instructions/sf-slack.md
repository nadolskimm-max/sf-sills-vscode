---
description: >-
  Integrates Salesforce with Slack including Slack apps, message formatting, interactive components, shortcuts, and the Salesforce for Slack package. Use when building Slack notifications from Salesforce, creating interactive Slack messages, configuring Salesforce for Slack, or building custom Slack apps that interact with Salesforce. Do NOT use for general integrations (use sf-integration) or email notifications (use sf-email).
---
...
2. Connect Slack workspace (Setup â†’ Slack)
...
â”śâ”€â”€ Entry: Opportunity.StageName changed to 'Closed Won'
â”śâ”€â”€ Get Records: Account details
â”śâ”€â”€ Action: SlackNotifier.sendMessage
â”‚   â”śâ”€â”€ channel: #sales-wins
â”‚   â””â”€â”€ message: "{!Account.Name} deal closed for {!$Record.Amount}"
â””â”€â”€ Update Record: Slack_Notified__c = true
```
...
|---|---|
| Deal alerts | Flow â†’ Slack webhook on Opp stage change |
| Case escalation | Flow â†’ Slack when Case priority = Critical |
| Approval in Slack | Salesforce for Slack package + approval process |
| Daily digest | Scheduled Flow â†’ aggregate â†’ Slack webhook |
| Interactive actions | Slack app â†’ Salesforce REST API |
